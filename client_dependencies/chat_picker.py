...
def create_frame(chat_picker_frame, chat_frame, user_values, server, key):
    def join_chat(*args):
        ...
        if request == SEND_GROUP_KEYS:
            # messagebox.showinfo("joining chat", "please wait while we encrypt your communication line")
            ...
            group_dh_key = pow(protocol_digest.bytes_to_int(group_dh_key), group_private_key.d, group_private_key.n)
            user_values.group_key = protocol_digest.int_to_bytes(group_dh_key)
    ...
    def open_new_chat():
        ...
        user_values.pin_code = protocol_digest.decrypt(server_response[HEADER_SIZE:].strip(b'\x00'), key).decode("utf-8")
        user_values.group_key = get_random_bytes(32)
        user_values.rsa_group_key = RSA.generate(2048)
        print(user_values.pin_code)
        chat_frame.tkraise()
        user_values.on_chat_raise()

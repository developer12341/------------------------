...
def create_frame(root,chat_frame,chat_picker_frame,user_values,server,key):
    ...
    def on_send(*args):
        msg = (f"<{user_values.username}>:" + chat_entry.get()).encode("utf-8")
        packets = protocols.Packet_Maker(SEND_MSG,user_values.group_key,msg)
            ...
            try:
                ...
                if packet_amount - packet_number > 1:
                    for _ in range(packet_amount-1):
                        packet = server.recv(PACKET_SIZE)
                        msg_queue.append(packet)
                msg = map(lambda packet: packet[HEADER_SIZE:],msg_queue).join()
